...
class XsctServer:
    ...
    def start_server(self, xsct_executable=None, port=PORT, verbose=False):
        ...
        # TODO: Modified to output tcl file since the -eval does not appear to
        # be working correctly with the xsdb server start

        # TODO: was: start_server_command = 'xsdbserver start -port {}'.format(port)

        stop_server_command = f'xsdbserver stop\n'
        start_server_command = f'xsdbserver start -host localhost -port {port}'
        with open('xsdbserver.tcl', 'w') as f:
            #f.write(stop_server_command)
            f.write(start_server_command)
            f.close()

        # TODO: was: start_command = '{} -eval "{}" -interactive'.format(xsct_executable, start_server_command)
        start_command = f'{xsct_executable} xsdbserver.tcl -interactive'
        logger.info('Starting xsct server: %s', start_command)
        if verbose:
            stdout = None
        else:
            stdout = subprocess.DEVNULL # TODO: was open(os.devnull, 'w'), but was not working as intended
        self._xsct_server = subprocess.Popen(start_command, stdout=stdout, stderr=stdout) # TODO: redirect stderr too
    ...
    def stop_server(self, wait=True):
        ...
        :return: None
        """
        print('[INFO] Stopping the xsct server')
...
class Xsct:
    ...
    def connect(self, host=HOST, port=PORT, timeout=10):
        ...
        # TODO: added logic to attempt connections for up to timeout
        connected = False
        starttime = time.time()
        while not connected:
            try:
                ...
                connected = True
            except (ConnectionRefusedError, ConnectionError, ConnectionAbortedError, ConnectionResetError, OSError):
    ...
    def close(self):
        ...
        :return: None
        """
        print('[INFO] Closing xsct socket')
        self._socket.close()
    ...
    def recv(self, bufsize=65536, timeout=None):
    ...
    def do(self, command, verbose = False, timeout = None, recv_delay = None):
        ...
        """
        if verbose:
            print(f'[INFO] xsct : {command}')
        logger.info('Sending command: %s ...', repr(command))
        command += xsct_line_end
        self.send(command)
        if recv_delay is not None:
            time.sleep(recv_delay)
        ans = self.recv(timeout = timeout)
        if not isinstance(ans,list):
            ans = [ans]

        if ans[0].startswith('okay'):
            ret = ans[0][5:].split('\\n')
            # if len(ans) > 1:
            #     for a in ans[1:]:
            #         ret.append(a.split('\\n'))
        ...
        if ans[0].startswith('error'):
            raise PyXilException(ans[0][6:])

        raise PyXilException('Illegal start-string in protocol. Answer is: ' + ans[0])
