# Rohde & Schwarz Fixed Power Supply
...
class NGX200(PowerSupply):
    USB_PID = '0001'     # Fake ID for mocking, see derived classes for real IDs
    NUM_CHANNELS = 1        # Min channels when generic
    MAX_VOLTAGE = 20.05
    MIN_CURRENT = 0.01  # Unique to this device, will be silent about it
    MAX_CURRENT = 6.01  # When <= 6V, 3.01 A when > 6V; see autoresponse in real device

    def __init__(self,*args, **kwargs):
        super(NGX200, self).__init__(*args, **kwargs)

        self.measure_delay = 0.3    # Emprically found, can be adjusted later as needed
    ...
    def current_setpoint(self, channel, amps = None):
        ...
        current_limit = self.MAX_CURRENT
        ...
        self.command(f'CURRent {amps}')      # Sets the voltage
    ...
    def output(self, channel = None, state = None):
        ...
        if self.coupling_mode is self.CouplingMode.EXCLUSIVE:
            ...
            if state[0] is self.State.ON:
                ...
                for c in range(1, self.NUM_CHANNELS+1):
                    if c != channel[0]:
                        self.command(f'INSTrument OUT{c}')
                        self.command(f'OUTPut:SELect OFF')
        ...
        for c in channel:
            ...
            self.command(f'INSTrument OUT{c}')
            self.command(f'OUTPut:SELect {"ON" if self.state[c-1]==self.State.ON else "OFF"}')
            i += 1
        ...
        if (self.state[0] is PowerSupply.State.OFF) and (self.state[1:] == self.state[:-1]):
            ...
            self.command('OUTPut:GENeral:STATe OFF')
        else:
            # At least one channel is active, enable the output
            self.command('OUTPut:GENeral:STATe ON')

class NGL201(NGX200):
    USB_PID = '0001'     # Fake ID for mocking, Replace when real is known
    NUM_CHANNELS = 1

    def __init__(self,*args, **kwargs):
        super(NGL201, self).__init__(*args, **kwargs)
...
class NGM201(NGX200):
    ...
    def __init__(self,*args, **kwargs):
        super(NGM201, self).__init__(*args, **kwargs)
...
class NGL202(NGX200):
    ...
    def __init__(self,*args, **kwargs):
        super(NGL202, self).__init__(*args, **kwargs)

class NGM202(NGX200):
    USB_PID = '0197'  # Real ID
    NUM_CHANNELS = 2

    def __init__(self,*args, **kwargs):
        super(NGM202, self).__init__(*args, **kwargs)
